...
simulPPIWeibull <- function(m, alpha, beta, p)
{
  ...
  S_k = c(1, exp(-(T_k[-1]/alpha)**beta)) # j'enlève la première valeur pour la remplacer par 1
  ...
  for(i in 1:m-1)
  {
   ...
   S_k = c(S_k, 1, exp(-(T_k_temp[-1]/alpha)**beta))
...
print(result)
plot(result$T,result$S, type='o')
points(result$T_succes, result$S_succes, col='red')
cat("T succes:")
print(result$T_succes)
cat("S succes:")
print(result$S_succes)
```
