...
p <- 0.2 #on pose 0.7 la proba d'avoir une réparation parfaite
...
$$
Ainsi, $\lambda^{-1}(y)=\alpha y^{\frac{1}{\beta}}$. On l'implémente donc avec comme paramètre $\beta=2$ et $\alpha=1$.
...
simulPPIWeibull <- function(m, alpha, beta, p)
{
  ...
  S_k = c(exp(-(head(T_k,-1)/alpha)**beta),1) # j'enlève la première valeur pour la remplacer par 1
...
ggplot(df_result, aes(x = T, y = S)) +
  geom_point(color = "black", size = 2) +            # points
...
plot_MLE_S <-function(t,x_order,phi_hat) {
  ...
  df <- data.frame(t = t, S_hat = S_hat)
  ggplot(df, aes(x = t, y = S_hat)) +
...
r<-MLE_S(T,Z)
t<-seq(0,20,0.1)
plot_MLE_S(t,r$x_order,r$phi_hat)
```
