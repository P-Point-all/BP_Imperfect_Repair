...
print(T_vec)
print(Z_vec)
plot(cumsum(T_vec), Age_vec, type="l", col="blue", lwd=2,
...
legend("topright", legend=c("Évolution de l'âge","Rep parfaite" ,"Réparations"), col=c("blue","red", "black"), pch=c(NA, 19, 2), lty=c(1, NA, NA))
```
...
Traçons la fonction de survie.
On sait que la focntion de survie d'une réparation parfaite est $S_{perfect}(t)=S(t)^p$ donc on en déduit que la fonction de survie d'une réparation imparfaite est $S_{perfect}(t)=S(t)^{1-p}$.

```{r survie function, include=TRUE}
time_vec <- cumsum(T_vec)
S_parfait <- exp(-lambda * time_vec)  # Survie après réparation parfaite
S_imparfait <- exp(-lambda * (1 - p) * time_vec)  # Survie avec réparation imparfaite
S_Y <- exp(-lambda * p * time_vec) # Survie jusqu'à la prochaine réparation parfaite

plot(time_vec, S_parfait, type="l", col="blue", lwd=2,
     ylim=c(0,1), xlab="Temps", ylab="Probabilité de survie",
     main="Fonctions de survie sous le modèle BP")

lines(time_vec, S_Y, col="red", lwd=2, lty=2)  # Jusqu'à la réparation parfaite
lines(time_vec, S_imparfait, col="green", lwd=2, lty=3)  # Avec réparations imparfaites

# Ajouter une légende
legend("topright", legend=c("S_parfait (Réparations parfaites)", 
                            "S_Y (Jusqu'à réparation parfaite)", 
                            "S_imparfait (Réparations imparfaites)"),
...
```{r survie function, include=TRUE}
index_order <- order(Age_vec)
Age_vec_ord <- Age_vec[index_order]
Z_vec_ord <- Z_vec[index_order]
