...
## 2.Estimation des paramètres $(p,F)$
...
# II.Modelisation numerique 

## 1.Simulation d'un processus de Weibull

Les $Z_i$ suivent une loi Binomiale par leur définition: $\mathbb{P}(Z_i=1)=p$.

On simule ici le modèle. Pour ce faire, on commence par créer deux fonctions: $\textbf{simulPPh1}$ et $\textbf{invWeibull}$. La fonction $\textbf{simulPPh1}$ permet de simuler un processus de poisson homogène: on simule $k$ fois une variable aléatoire suivant la loi exponentielle de paramètre 1 ce qui nous donne une liste de nombres telle que $[n_1 , n_2,...,n_k]$. On renvoie ensuite les temps d'arrivés simulés (notés $A_k$ dans la fonction $\textbf{simulPPIWeibull}$ ) qui représentent la somme cumulée telle que $[n_1, n_1+n_2,... , n_1+...+n_k]$.
...
La fonction $\textbf{invWeibull}$ renvoie l'intensité cumulée inverse de Weibull: $\Lambda^{-1}$. Montrons comment nous l'avons trouvé:
...
## 2.Première simulation d'un processus de Brown-Prosschan 
...
## 3.Estimation du paramètre $p$ par maximum de vraisemblance
...
df <- data.frame(m = 1:1000, p_hat = p_hat)
  ...
  ggplot(df, aes(x = m, y = p_hat)) +
    ...
    labs(title = "Estimation de p̂ en fonction de m",
         x = "m",
         y = "p") +
    theme_minimal(base_size = 14)
...
## 4.Estimation de l'intensité du processus par plusieurs méthodes
### a.Estimation par maximum de vraisemblance ou neighbourhood MLE
...
On définit une nouvelle fonction $\textbf{simulPPIWeibull2}$, qui s'inspire fortement de la première simulation mais qui ne renvoie pas les mêmes choses. Ici on retourne les instants de pannes, la type de réparation associé à chaque panne ($=Z$) ainsi que les instants de réparations parfaites (sous ensemble de $T$).
```{r}
...
```{r}
res<-simulPPIWeibull2(10,4,2,0.2)
Z<-res$Z
T<-res$T 
print(res)
...
On implémente maintenant le calcul du MLE (ou neighbourhood MLE en fonction des cas) trouvé précedemment. La fonction $\textbf{MLE_S}$ renvoie les vecteurs $x_{()}$ et $\hat \phi$ permettant de construire le MLE. On a ensuite une deuxième fonction $\textbf{plot_MLE_S}$ qui permet de construire graphiquement le MLE en retournant ses valeurs pour une séquence de temps $t$.
```{r}
MLE_S <- function(T,Z)
{
  n<-length(Z)
...
ggplot(df, aes(x = t, y = S_hatBP)) +
  ...
  theme_minimal(base_size = 14)
```
### b.Estimation de l'intensité du processus en utilisant les temps de pannes à la suite d'une réparation parfaite
...
ggplot(df, aes(x = t, y =S_hat_e1)) +
  ...
  theme_minimal(base_size = 14)
```
### c.Estimation de l'intensité du processus avec uniquement les temps de réparations parfaites
...
```{r Comparaison_visuelle, eval=FALSE}
t<-seq(0,10,0.01)
plot(SWeibull(t, 4,2), col = "black")
lines(S_hatBP, col = "red")
lines(S_hat_e1, col = "blue")
lines(S_hat_e2, col = "green")
```
